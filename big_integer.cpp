...
big_integer::big_integer(): data(std::vector<uint32_t>(1, 0)) , sign(0)
{}
...
void big_integer::mul(uint32_t val)
{
    ...
    while (tmp)
    {
        data.push_back((uint32_t) (tmp & UINT_MAX));
        tmp >>= 32;
    }
}
...
void big_integer::add(big_integer const& b) // только для чисел одинакового знака, у них должны быть либо одинаковый размер либо у нашего меньше
{
    uint64_t tmp = 0;
    resize(b.data.size());
    ...
    while (tmp)
    {
        ...
        data[pos] = (uint32_t) (tmp & UINT_MAX);
        tmp >>= 32;
    }
    resize(0);
}
...
big_integer operator*(big_integer a, big_integer const& b)
{
    ...
    for (size_t i = 0; i < a.data.size(); ++i)
    {
        for (size_t j = 0; j < b.data.size(); ++j)
        {
            ...
            res.data[i + j] = tmp & UINT_MAX;
            tmp >>= 32;
        }
        size_t pos = i + b.data.size();
        while (tmp) {
            tmp += (uint64_t) res.data[pos];
            res.data[pos++] = tmp & UINT_MAX;
            tmp >>= 32;
        }
    }
    res.resize(0);
...
void big_integer::resize(size_t expected_size = 0) {// добавляет в начало нули, чтобы сделать одинакого размера || удаляет лишние нули
    if (expected_size == 0) {
...
{
    if (data.back() != 0) {//если число отрицательное
        invert();
        add(1);
        sign = -1;
    }
...
{
    return (a.sign == b.sign && a.data == b.data) || (a.data.size() == 0 && b.data.size() == 0) || (a.sign == 0 && b.sign == 0);
}
